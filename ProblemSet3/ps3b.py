...
class Patient(object):
    ...
    def update(self):
        ...
        currentPopDensity = self.getTotalPop() / self.maxPop
        
        temp_viruses = self.viruses[:]
        for virus in self.viruses:
            try:
                temp_viruses.append(virus.reproduce(currentPopDensity))
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    pylab.plot(avg_population_at_each_time_step_list)
#    pylab.title("Population simulation without drug")
#    pylab.legend("Population changes for timesteps", loc="best")
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        if random.random() < self.maxBirthProb * (1 - popDensity):
            temp_resistances = (self.resistances).copy()
...
#==============================================================================
# # tests from bribre (Community TA)
# def sim_test6(trials):
#     results = []
#     num_children = 10
#     for i in range(trials):
#         virus = ResistantVirus(1.0, 0.0, 
#                                {"drug1": True, "drug2": True,
#                                 "drug3": True, "drug4": True,
#                                 "drug5": True, "drug6": True}, 0.5)
#         num_drugs = len(virus.getResistances())
#         
#         resistance = 0
#         for num in range(num_children):
#             child = virus.reproduce(0, [])
#             resistance += sum((1 for drug in child.getResistances()
#                                     if child.getResistances()[drug]))
#         resistance /= num_children
#         results.append(resistance/num_drugs)
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for time_step in range(TIME_STEPS):
        ...
        for patient in patients:
            total_pops.append(patient.update())
            guttr_pops.append(patient.getResistPop([GUTT]))        
        avg_total_pops.append(sum(total_pops) / len(total_pops))
        avg_guttr_pops.append(sum(guttr_pops) / len(guttr_pops))
                       
    # Add drug "guttagonol"
    for patient in patients:
        patient.addPrescription(GUTT)
