...
def r_squared(y, estimated):
    ...
    """
    error = ((np.array(estimated) - y)**2).sum()
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        ...
        model = pylab.polyfit(x, y, len(model)-1)
        print(model)
        estYs = pylab.polyval(model, x)
        rSquared = r_squared(y, estYs)
        print(estYs)
        print(rSquared)
        pylab.title("Temperature Changes Over Years With\n" + \
                    "Model: " + str(model) + "\n" + \
                    "R-square: " + str(round(rSquared, 4)))
        pylab.plot(x, estYs, "r-", label="Best fit curve")
